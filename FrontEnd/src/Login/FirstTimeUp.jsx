    ...
    return (
        <>
            {error === 'Email is already taken.' && (
                <div className="fixed top-1 left-1/2 transform -translate-x-1/2 z-50 bg-red-500 text-white p-3 w-11/12 md:w-1/3 text-center rounded-lg shadow-lg">
                    <p>{error}</p>
                </div>
            )}
            {showAlert && (
                <div className="fixed top-1 left-1/2 transform -translate-x-1/2 z-50 bg-green-500 text-white p-3 w-11/12 md:w-1/3 text-center rounded-lg shadow-lg">
                    <p>{resultat}</p>
            ...
            <div className="flex min-h-screen items-center justify-center bg-gray-100">
                <div className="flex flex-col md:flex-row w-full max-w-5xl rounded-lg bg-white shadow-lg">
                    {/* Left Side - Car Image */}
                    <div className="w-full md:w-1/2 hidden md:flex items-center justify-center rounded-l-lg">
                        <div
                            className="absolute left-56 inset-0 bg-cover bg-center opacity-10 w-1/3 h-full"
                            style={{ backgroundImage: 'url(/Pictures/Logo.svg)' }}
                        ></div>
                        <img
                            src={carImage}
                            alt="Car"
                            className="object-cover rounded-l-lg w-full mix-blend-multiply"
                        />
                    </div>
                            ...
                            <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full mt-1 px-4 py-2 border ${
                                            error === 'Name is required.' ? 'border-red-500' : 'border-gray-300'
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full mt-1 px-4 py-2 border ${
                                            error === 'Invalid email format.' || error === 'Email is already taken.'
                                                ? 'border-red-500'
                                                : 'border-gray-300'
                                        } rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500`}
                                    />
