...
function Notification() {
    ...
    {isOpen && (
        <div ref={menuRef} className="bg-white sm:w-48 md:w-64 absolute right-4 sm:right-10 md:right-36 top-14 border border-gray-200 rounded-lg shadow-lg">
                ...
                {notifications.length > 0 ? (
                    notifications.map((notification, index) => (
                        <li
                            key={index}
                            onClick={() => handleNotificationClick(notification)}
                            className={`px-4 py-2 text-sm sm:text-base text-gray-800 hover:bg-gray-100 cursor-pointer ${notification.status === 'Unread' ? 'font-bold' : ''}`}
                        >
                            {notification.message.slice(0, 20)}{notification.message.length > 10 ? '...' : ''}
                            {notification.status === 'Unread' && (
                                <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full"></span>
